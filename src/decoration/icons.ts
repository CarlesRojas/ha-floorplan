import {
  faBars,
  faBath,
  faBed,
  faBell,
  faBlender,
  faBowlFood,
  faBoxArchive,
  faBreadSlice,
  faCamera,
  faChair,
  faClock,
  faComputer,
  faCouch,
  faDoorClosed,
  faDoorOpen,
  faDroplet,
  faFan,
  faFilm,
  faFire,
  faFireFlameCurved,
  faFish,
  faGripLines,
  faHotTubPerson,
  faImage,
  faKitchenSet,
  faLaptop,
  faLeaf,
  faLightbulb,
  faLock,
  faMobileScreen,
  faPaw,
  faPlug,
  faRobot,
  faSeedling,
  faShirt,
  faShower,
  faSnowflake,
  faSpa,
  faTable,
  faTableCells,
  faTemperatureHalf,
  faTree,
  faTv,
  faVolumeHigh,
  faWarehouse,
  faWaterLadder,
  faWind,
  faWindowMaximize,
  type IconDefinition,
} from '@fortawesome/free-solid-svg-icons'

// Icon per decoration kind, with a fallback per family.
const BY_KIND: Record<string, IconDefinition> = {
  sofa: faCouch,
  dining_chair: faChair,
  office_chair: faChair,
  stool: faChair,
  bench: faChair,
  pouf: faChair,
  light_globe: faLightbulb,
  ceiling_extractor: faWind,
  bookshelf: faBoxArchive,
  wardrobe: faShirt,
  fridge: faSnowflake,
  oven: faFire,
  hob: faFire,
  dishwasher: faKitchenSet,
  kitchen_sink: faDroplet,
  microwave: faKitchenSet,
  washing_machine: faShirt,
  dryer: faShirt,
  toilet: faBath,
  basin: faDroplet,
  bathtub: faBath,
  shower: faDroplet,
  towel_rail: faShirt,
  half_wall: faGripLines,
  rug: faImage,
  plant_large: faSeedling,
  plant_small: faSeedling,
  plant_wall: faLeaf,
  wall_mirror: faImage,
  wall_clock: faClock,
  vase: faSeedling,
  books: faBoxArchive,
  curtain: faBars,
  tv: faTv,
  tv_wall: faTv,
  soundbar: faVolumeHigh,
  speaker: faVolumeHigh,
  floor_speaker: faVolumeHigh,
  monitor: faTv,
  game_console: faTv,
  projector: faTv,
  projector_portable: faTv,
  projector_screen: faTv,
  radiator: faFire,
  ac_unit: faSnowflake,
  fan_ceiling: faFan,
  fan_floor: faFan,
  air_purifier: faWind,
  humidifier: faDroplet,
  thermostat: faTemperatureHalf,
  blind: faBars,
  window: faWindowMaximize,
  door: faDoorOpen,
  sliding_door: faDoorClosed,
  garage_door: faWarehouse,
  awning: faBars,
  camera: faCamera,
  doorbell: faBell,
  motion_sensor: faBell,
  smoke_detector: faBell,
  alarm_panel: faBell,
  smart_lock: faLock,
  air_quality: faWind,
  vacuum_robot: faRobot,
  pet_feeder: faPaw,
  water_heater: faTemperatureHalf,
  fireplace: faFireFlameCurved,
  christmas_tree: faTree,
  aquarium: faFish,
  hot_tub: faHotTubPerson,
  pool: faWaterLadder,
  sprinkler: faShower,
  toaster: faBreadSlice,
  cooking_robot: faBlender,
  air_fryer: faBowlFood,
  pc_tower: faComputer,
  laptop: faLaptop,
  wireless_charger: faMobileScreen,
  projector_ust: faFilm,
  louvred_pergola: faTableCells,
  sauna: faSpa,
}

const BY_FAMILY: Record<string, IconDefinition> = {
  light: faLightbulb,
  seating: faCouch,
  table: faTable,
  storage: faBoxArchive,
  bed: faBed,
  kitchen: faKitchenSet,
  laundry: faShirt,
  bathroom: faBath,
  decor: faImage,
  media: faTv,
  climate: faTemperatureHalf,
  cover: faWindowMaximize,
  security: faBell,
  utility: faPlug,
  outdoor: faTree,
}

export const decorationIcon = (kindId: string, family: string) => BY_KIND[kindId] ?? BY_FAMILY[family] ?? faBoxArchive

export const FAMILY_LABELS: Record<string, string> = {
  light: 'Lights',
  seating: 'Seating',
  table: 'Tables',
  storage: 'Storage',
  bed: 'Beds',
  kitchen: 'Kitchen',
  laundry: 'Laundry',
  bathroom: 'Bathroom',
  decor: 'Decor',
  media: 'Media',
  climate: 'Climate',
  cover: 'Windows and doors',
  security: 'Security and sensors',
  utility: 'Smart home',
  outdoor: 'Garden and outdoor',
}
