...
- A robot vacuum comes with the dock it charges on. The dock stays where the piece was placed, since that is the thing plugged into the wall, and the robot leaves it while the vacuum is cleaning: it sweeps the room in long parallel passes, each one cut back to the stretch that is clear of the furniture standing on the floor, and retraces its way back to the dock when the vacuum stops. A press on either the robot or the dock does what a press on the piece does.
