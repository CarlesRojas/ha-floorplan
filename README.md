...
- An item with more than one look and no entity behind it can be tried in each of them, in the editor only. Its panel has a Try its states section with a switch for on and off, a slider for each level (brightness, speed, how far a cover is open, a blind's slats, a window's tilt) and, for lights, a default, white or color tint. Clicking the item in 3D switches it too, the way it would act on an entity, and a cover opens all the way or shuts. Nothing tried is saved, and Reset puts the item back. An item with an entity shows the entity's state instead.
